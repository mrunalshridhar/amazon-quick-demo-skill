...
## ⚠️ CRITICAL — Pre-Write Safety Check

Before writing to OUTPUT_PATH, check if `amazon-quick-features.pptx` or `amazon-quick-features.html`
already exist there. If they do, it means a PREVIOUS demo's files are still present.

- Read `OUTPUT_PATH/demo-manifest.json` to get the previous `demo_name`
- Rename existing files: `amazon-quick-features.pptx` → `<previous-demo-prefix>-amazon-quick-features.pptx`
- Then write your new files normally
